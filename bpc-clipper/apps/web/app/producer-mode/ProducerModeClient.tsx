...
export function ProducerModeClient({ projectId }: { projectId?: string }) {
  ...
  async function analyzeStream() {
    ...
    try { await withActiveSource(async (sourceId) => { const analysis = await analyzeGameSenseStream(sourceId, true, 0.30); refreshEvidence(); setStatus('Titan found evidence. Ranking the strongest GameSense moments...'); const generated = await generateGameSenseCandidates(projectId, sourceId, true); setCandidates(generated.candidates); const { audio_spike_count, visual_scene_change_count } = analysis.summary; setStatus(`Titan analyzed the stream and ranked ${generated.generated_count} clip candidate(s) from ${audio_spike_count} audio reaction spike(s) and ${visual_scene_change_count} visual action signal(s).`); }, 'No source exists for this project yet.'); }
  ...
  return <>
    <section className="card" style={{ marginTop: 24 }}>
      ...
      {projectId && <label style={{ display: 'grid', gap: 6, marginTop: 12 }}><strong>Active source</strong><select value={activeEvidenceSourceId || ''} onChange={(event) => { setActiveEvidenceSourceId(event.target.value || null); refreshEvidence(); }} disabled={!sources.length}><option value="">Select a source</option>{sources.map((source) => <option key={source.source_id} value={source.source_id}>{sourceLabel(source)}</option>)}</select></label>}
      <div className="button-row" style={{ marginTop: 12 }}><button className="button" type="button" onClick={analyzeStream} disabled={isAnalyzingStream || !projectId || !activeEvidenceSourceId}>{isAnalyzingStream ? 'Analyzing + Ranking...' : 'Analyze Stream + Find Clips'}</button><button className="button secondary" type="button" onClick={generateGameSense} disabled={isGeneratingGameSense || !projectId || !activeEvidenceSourceId}>{isGeneratingGameSense ? 'Finding Gaming Moments...' : 'Generate GameSense Clips'}</button><button className="button secondary" type="button" onClick={rescoreProject} disabled={isRescoring || !projectId}>{isRescoring ? 'Rescoring...' : 'Rescore with Titan Brain'}</button></div>
      <details style={{ marginTop: 12 }}><summary style={{ cursor: 'pointer' }}>Advanced detection controls</summary><div className="button-row" style={{ marginTop: 10 }}><button className="button secondary" type="button" onClick={scanAudioForReactions} disabled={isScanningAudio || !projectId || !activeEvidenceSourceId}>{isScanningAudio ? 'Scanning Stream Audio...' : 'Scan Audio for Reactions'}</button><button className="button secondary" type="button" onClick={scanVisualForAction} disabled={isScanningVisual || !projectId || !activeEvidenceSourceId}>{isScanningVisual ? 'Scanning Stream Visuals...' : 'Scan Visual Action'}</button></div></details>
