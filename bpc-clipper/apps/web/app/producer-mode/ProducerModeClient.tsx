...
function ScoreBreakdownPanel({ breakdown }: { breakdown?: ScoreBreakdown }) {
  ...
  if (gameEvidence.length) {
    return <div className="card" style={{ marginTop: 12 }}><h3>Titan GameSense</h3>{breakdown?.overall && <p><strong>Moment score:</strong> {breakdown.overall.score} — {breakdown.overall.explanation}</p>}<p><strong>Aligned evidence:</strong> {gameEvidence.length} signal(s)</p>{gameEvidence.map((signal, index) => <p key={`${signal.event_type}-${index}`} style={{ margin: '6px 0', opacity: 0.9 }}>{formatCategory(signal.event_type)} · {formatCategory(signal.modality)} · intensity {signal.intensity}</p>)}</div>;
  }
  ...
  return <>
    ...
    <section style={{ display: 'grid', gap: 18, marginTop: 24 }}>
      {sortedCandidates.map((candidate) => {
        ...
        return <article className="card candidate" key={candidate.candidate_id}>
          <div><div className="score">{candidate.score}</div><div className="badge">{formatCategory(candidate.category)}</div></div>
          <div><h2>{candidate.title}</h2><p><strong>Source time:</strong> {formatTime(candidate.start_seconds)} - {formatTime(candidate.end_seconds)}</p><p>{candidate.excerpt}</p><p><strong>Why it ranked:</strong> {candidate.explanation}</p><ScoreBreakdownPanel breakdown={candidate.score_breakdown} />{candidate.risk_flags.length > 0 && <p><strong>Risk flags:</strong> {candidate.risk_flags.join(', ')}</p>}
            {workflow && <div className="card" style={{ marginTop: 12 }}><p><strong>Workflow:</strong> {workflow.status}</p>{workflow.renderJob && <p><strong>Render Job:</strong> {workflow.renderJob.status} / {workflow.renderJob.progress}%</p>}{workflow.exportRecord && <><p><strong>Export Status:</strong> {workflow.exportRecord.status}</p><ExportLinks exportRecord={workflow.exportRecord} /></>}{workflow.error && <p style={{ color: '#ff8a8a' }}><strong>Error:</strong> {workflow.error}</p>}</div>}
          </div>
          <div className="button-row"><button className="button" type="button" onClick={() => approveAndQueueRender(candidate)}>Approve + Queue Render</button><button className="button secondary" type="button" onClick={() => approveAndRenderNow(candidate)}>Render Now</button></div>
        </article>;
      })}
    </section>
  </>;
