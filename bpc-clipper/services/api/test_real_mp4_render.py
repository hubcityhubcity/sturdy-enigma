...
from render_scaffold import SpeakerFocusPlanRequiredError
...
class RealMp4RenderTests(unittest.TestCase):
    ...
    def _make_editing_records(self, crop_mode: str = "center_focus"):
        ...
        edit = EditTimeline(id="edit-1", project_id=project.id, candidate_clip_id=candidate.id, start_seconds=0.0, end_seconds=2.0, hook_text="Real moment", caption_preset="bpc_clean_editorial", crop_mode=crop_mode, status="ready_to_render", settings={})
        ...
        db.commit()
        return db, export, edit

    def test_real_media_source_renders_a_playable_vertical_mp4_with_manual_framing(self):
        db, export, edit = self._make_editing_records(crop_mode="center_focus")
    ...
    def test_speaker_focus_cannot_silently_fall_back_to_center_crop(self):
        db, export, edit = self._make_editing_records(crop_mode="speaker_focus")
        with self.assertRaises(SpeakerFocusPlanRequiredError):
            render_export_with_best_source(db, export, edit)
        db.close()
